...
public class MazeGrid implements MazeModel {
    ...
    public boolean hasWall(int row, int col, int nRow, int nCol) {
        int mr = (row + nRow) / 2, mc = (col + nCol) / 2;
    ...
    /**
     * 兼容适配器：将物理坐标的拆墙请求映射到 carveCell。
     * GameController.breakWall 调用此方法，目标 (r2,c2) 为需要凿开的物理墙格。
     */
    public void removeWall(int r1, int c1, int r2, int c2) {
        carveCell(r2, c2);
    }
